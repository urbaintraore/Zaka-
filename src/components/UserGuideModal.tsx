...
import { X, BookOpen, Star, Sparkles, Shield, DollarSign, Users, Info, Award, Megaphone, Building2, Utensils, Scissors, Target, Download, Printer, FileText, Check } from 'lucide-react';
import { downloadGuidePDF, downloadGuideMarkdown, printGuideDirectly } from '../utils/downloadGuide';
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-950 w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        {/* Header */}
        <div className="p-5 md:p-6 border-b border-gray-150 dark:border-gray-900 bg-gradient-to-r from-orange-500 to-red-600 text-white flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title="Télécharger le guide au format PDF"
              className="px-3 py-1.5 bg-white/20 hover:bg-white/30 active:scale-95 text-white rounded-xl transition-all text-xs font-bold flex items-center gap-1.5 cursor-pointer shadow-sm"
            >
        ...
        {/* Footer */}
        <div className="p-4 sm:p-5 border-t border-gray-150 dark:border-gray-900 bg-gray-50 dark:bg-gray-900/40 flex flex-wrap items-center justify-between gap-3 flex-shrink-0">
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={handleDownloadPDF}
              className="px-3.5 py-2 bg-orange-600 hover:bg-orange-700 active:scale-95 text-white font-bold rounded-xl transition-all cursor-pointer text-xs flex items-center gap-1.5 shadow-sm"
              title="Télécharger le guide au format PDF"
            >
              {downloadSuccess === 'PDF' ? <Check className="w-3.5 h-3.5 text-white" /> : <Download className="w-3.5 h-3.5" />}
              <span>{downloadSuccess === 'PDF' ? 'PDF Téléchargé !' : 'Télécharger (PDF)'}</span>
            </button>

            <button
              onClick={handleDownloadMarkdown}
              className="px-3 py-2 bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200 font-semibold rounded-xl border border-gray-200 dark:border-gray-700 transition-all cursor-pointer text-xs flex items-center gap-1.5"
              title="Télécharger le guide au format Markdown (.md)"
            >
              {downloadSuccess === 'Markdown' ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <FileText className="w-3.5 h-3.5" />}
              <span>Markdown</span>
            </button>

            <button
              onClick={printGuideDirectly}
              className="px-3 py-2 bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200 font-semibold rounded-xl border border-gray-200 dark:border-gray-700 transition-all cursor-pointer text-xs flex items-center gap-1.5"
          ...
          <button
            onClick={onClose}
            className="py-2 px-5 bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200 font-bold rounded-xl active:scale-[0.98] transition-all cursor-pointer text-xs"
          >
            Fermer
          </button>
        </div>
