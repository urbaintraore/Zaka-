...
export function AvisUtilisateurs({ establishmentId }: AvisUtilisateursProps) {
  const { reviews, currentUser, addReview, setGlobalError, establishments } = useAppStore();
  const [rating, setRating] = useState(5);
  const [ratingPlats, setRatingPlats] = useState(5);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await addReview({
      ...
      establishmentId,
      rating: isRestaurant ? Math.round((ratingPlats + ratingService) / 2 * 10) / 10 : rating,
      ratingPlats: isRestaurant ? ratingPlats : undefined,
      ratingService: isRestaurant ? ratingService : undefined,
      comment,
        ...
        {!isRestaurant ? (
          ...
          <div className="flex flex-col gap-3 mb-4">
            <div className="flex items-center justify-between">
              <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">Qualité des plats</span>
              <div className="flex items-center gap-1">
                {[1,2,3,4,5].map(star => (
                  <Star 
                    key={star} 
                    className={`w-5 h-5 cursor-pointer transition-transform hover:scale-110 ${star <= ratingPlats ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300 dark:text-gray-700'}`}
                    onClick={() => setRatingPlats(star)}
                  />
                ))}
              </div>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">Qualité du service</span>
              <div className="flex items-center gap-1">
                {[1,2,3,4,5].map(star => (
                  <Star 
                    key={star} 
                    className={`w-5 h-5 cursor-pointer transition-transform hover:scale-110 ${star <= ratingService ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300 dark:text-gray-700'}`}
                ...
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    <div className="flex text-yellow-400">
                        {[...Array(Math.round(review.rating))].map((_,i) => <Star key={i} className="w-3.5 h-3.5 fill-yellow-400" />)}
                    </div>
                    <span className="text-xs font-semibold text-gray-800 dark:text-gray-200">
                      {review.clientName || "Utilisateur Zaka"}
                    </span>
                  </div>
                  {(review.ratingPlats !== undefined || review.ratingService !== undefined) && (
                    <div className="flex items-center gap-2 text-[10px] text-gray-500 dark:text-gray-400 font-medium">
                      <span>Plats : {review.ratingPlats || review.rating} ★</span>
                      <span>·</span>
                      <span>Service : {review.ratingService || review.rating} ★</span>
                    </div>
                  )}
                </div>
